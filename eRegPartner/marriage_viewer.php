<?php
/**
 * Marriage Viewer - Compact for 15" Monitors
...
if (empty($registryNum)) {
    header('Location: marriage_transmission.php');
    exit;
}
...
<head>
    ...
    <title>Marriage Record - <?php echo htmlspecialchars($registryNum); ?></title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .navbar-left h4 { margin: 0; font-size: 1rem; color: white; }
        .navbar-left small { color: rgba(255, 255, 255, 0.7); font-size: 0.8rem; }
        ...
        .btn-send-nav { background: #667eea; color: white; }
        .btn-back-nav { background: #1e293b; color: white; }
        
        .main-container { padding: 15px 30px; max-width: 1400px; margin: 0 auto; }
        ...
        .send-history-box.show { display: block; }
        ...
        .history-item-label { color: #64748b; font-size: 0.7rem; }
        .history-item-value { color: #ffffff; font-weight: 500; }
        .history-item-value.mobile { color: #00ff00; font-weight: 700; }
        ...
        .no-history { text-align: center; padding: 40px 20px; display: none; }
        .no-history.show { display: block; }
        .no-history h3 { color: #ef4444; font-size: 1.2rem; margin: 0; }
...
<body>
    ...
    <nav class="navbar">
        <div class="navbar-left">
            <h4><i class="fas fa-ring me-2"></i>MARRIAGE RECORD</h4>
            ...
            <div>
                ...
                <div class="info-card">
                    <div class="card-header">
                        <i class="fas fa-heart me-2"></i>MARRIAGE INFORMATION
                    </div>
                    <div class="card-body">
                        <div class="info-row">
                        ...
                        <div class="info-row">
                            ...
                            <div class="info-value" id="marriage-date">-</div>
                        </div>
                        <div class="info-row">
                            <div class="info-label">Marriage Place:</div>
                            <div class="info-value" id="marriage-place">-</div>
                        </div>
                    </div>
                </div>

                <!-- HUSBAND AND WIFE SIDE BY SIDE -->
                <div class="husband-wife-grid">
                    <!-- HUSBAND'S INFORMATION -->
                    <div class="info-card">
                        <div class="card-header">
                            <i class="fas fa-male me-2"></i>HUSBAND'S INFORMATION
                        </div>
                        <div class="card-body">
                            <div class="info-row">
                                <div class="info-label">First Name:</div>
                                <div class="info-value" id="h-first-name">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Middle Name:</div>
                                <div class="info-value" id="h-middle-name">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Last Name:</div>
                                <div class="info-value" id="h-last-name">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Date of Birth:</div>
                                <div class="info-value" id="h-birth-date">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Age:</div>
                                <div class="info-value" id="h-age">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Mother :</div>
                                <div class="info-value" id="h-mother">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Father :</div>
                                <div class="info-value" id="h-father">-</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="info-card">
                        <div class="card-header">
                            <i class="fas fa-female me-2"></i>WIFE'S INFORMATION
                        </div>
                        <div class="card-body">
                            <div class="info-row">
                                <div class="info-label">First Name:</div>
                                <div class="info-value" id="w-first-name">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Middle Name:</div>
                                <div class="info-value" id="w-middle-name">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Last Name:</div>
                                <div class="info-value" id="w-last-name">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Date of Birth:</div>
                                <div class="info-value" id="w-birth-date">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Age:</div>
                                <div class="info-value" id="w-age">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Mother :</div>
                                <div class="info-value" id="w-mother">-</div>
                            </div>
                            <div class="info-row">
                                <div class="info-label">Father :</div>
                                <div class="info-value" id="w-father">-</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    async function loadRecord() {
        ...
        try {
            const response = await fetch(`registry_api.php?action=get_record&type=marriage&registry_num=${encodeURIComponent(REGISTRY_NUM)}`, {
                headers: {
    ...
    function displayRecord(data) {
        ...
        document.getElementById('marriage-date').textContent = data.MarriageDate ? formatDate(data.MarriageDate) : '-';
        document.getElementById('marriage-place').textContent = data.MarriagePlaceMunicipality || '-';
        
        // Husband Info
        document.getElementById('h-first-name').textContent = data.HFirstName || '-';
        document.getElementById('h-middle-name').textContent = data.HMiddleName || '-';
        document.getElementById('h-last-name').textContent = data.HLastName || '-';
        document.getElementById('h-birth-date').textContent = data.HBirthDate ? formatDate(data.HBirthDate) : '-';
        document.getElementById('h-age').textContent = data.HAge || '-';
        document.getElementById('h-mother').textContent = data.Hmother_name || '-';
        document.getElementById('h-father').textContent = data.Hfather_name || '-';
        
        // Wife Info
        document.getElementById('w-first-name').textContent = data.WFirstName || '-';
        document.getElementById('w-middle-name').textContent = data.WMiddleName || '-';
        document.getElementById('w-last-name').textContent = data.WLastName || '-';
        document.getElementById('w-birth-date').textContent = data.WBirthDate ? formatDate(data.WBirthDate) : '-';
        document.getElementById('w-age').textContent = data.WAge || '-';
        document.getElementById('w-mother').textContent = data.Wmother_name || '-';
        document.getElementById('w-father').textContent = data.Wfather_name || '-';
    ...
    function goBack() {
        window.location.href = 'marriage_transmission.php';
    }
    </script>
</body>
</html>