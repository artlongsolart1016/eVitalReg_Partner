<?php
/**
 * Death Transmission - Cloned from Birth
 * Same logic, different columns
 */
...
<head>
   <meta charset="UTF-8">
    ...
    <title>Death Records - Partner Transmission System</title>
...
<body>
   <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        ...
        <div class="collapse navbar-collapse" id="navbarContent">
           ...
           <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <!-- Statistics Reports Dropdown -->
                  <li class="nav-item">
            <a class="nav-link" href="death_statistics_export.php">
    ...
    <div class="app-container">
        <div class="controls-section">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-2">
                <div class="d-flex align-items-center gap-2">
                    <i class="fas fa-cross" style="font-size: 2.5rem; color: var(--blue-light);"></i>
                    <h5 class="mb-0" style="font-size: 1.2rem;">Death Records - Unregistered</h5>
                </div>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb" style="background: transparent; padding: 0; margin: 0; font-size: 0.85rem;">
                    ...
                    <li class="breadcrumb-item active" style="color: #ccc;">Death Transmission</li>
                </ol>
            </nav>
        </div>
        ...
        <div class="table-scroll-container">
            <table>
                <thead>
                    <tr>
                        <th colspan="7" class="band-header">Death Details</th>
                        <th colspan="2" class="band-header">Parent's Information</th>
                        <th colspan="3" class="band-header">Status</th>
                    </tr>
                    <tr>
                        <th>Registry #</th>
                        <th>First Name</th>
    ...
    async function loadRecords() {
        ...
        tbody.innerHTML = '<tr><td colspan="12" class="loading"><i class="fas fa-spinner fa-spin"></i> Loading...</td></tr>';
        
        try {
            const response = await fetch(`registry_api.php?action=list_records&type=death&page=${currentPage}&per_page=50&search=${encodeURIComponent(search)}`, {
                headers: {
            ...
            tbody.innerHTML = records.map(r => `
                <tr class="clickable-row" onclick="viewRecord('${escapeHtml(r.registry_num)}')" style="cursor: pointer;" title="Click to view/send">
                    ...
                    <td>${escapeHtml(r.date_of_death)}</td>
            ...
            tbody.innerHTML = `<tr><td colspan="12" class="text-center text-danger">Error: ${escapeHtml(error.message)}</td></tr>`;
        }
    }
    ...
    async function loadStats() {
        try {
            const response = await fetch('registry_api.php?action=get_stats&type=death', {
                headers: {
                    'X-CSRF-Token': CSRF_TOKEN
                }
            });
    ...
    function viewRecord(registryNum) {
        window.location.href = `death_viewer.php?id=${encodeURIComponent(registryNum)}`;
    }
