<?php
/**
 * Marriage Transmission - Cloned from Birth
 * Shows husband, wife, marriage details
 */
...
<head>
    ...
    <title>Marriage Records - Partner Transmission System</title>
...
<body>
        ...
        <div class="collapse navbar-collapse" id="navbarContent">
           ...
           <ul class="navbar-nav me-auto mb-2 mb-lg-0">
            ...
            <a class="nav-link" href="birth_statistics_export.php">
                <i class="fas fa-chart-bar me-1"></i> Statistics Reports
            </a>
        </li>

        <!-- Death Main Menu -->
        <li class="nav-item">
            <a class="nav-link" href="birth_transmission.php">
    ...
    <div class="app-container">
        <div class="controls-section">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-2">
                <div class="d-flex align-items-center gap-2">
                    <i class="fas fa-ring" style="font-size: 2.5rem; color: var(--blue-light);"></i>
                    <h5 class="mb-0" style="font-size: 1.2rem;">Marriage Records - Unregistered</h5>
                </div>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb" style="background: transparent; padding: 0; margin: 0; font-size: 0.85rem;">
                    ...
                    <li class="breadcrumb-item active" style="color: #ccc;">Marriage Transmission</li>
                </ol>
            </nav>
        </div>
        ...
        <div class="table-scroll-container">
            <table>
                <thead>
                    <tr>
                        <th rowspan="2" class="band-header">Registry</th>
                        <th rowspan="2" class="band-header">Marriage Date</th>
                        <th colspan="5" class="band-header">Husband Details</th>
                        <th colspan="5" class="band-header">Wife Details</th>
                        <th colspan="4" class="band-header">Status</th>
                    </tr>
                    <tr>
                        <th>First Name</th>
                        <th>Middle Name</th>
    ...
    async function loadRecords() {
        ...
        tbody.innerHTML = '<tr><td colspan="15" class="loading"><i class="fas fa-spinner fa-spin"></i> Loading...</td></tr>';
        
        try {
            const response = await fetch(`registry_api.php?action=list_records&type=marriage&page=${currentPage}&per_page=50&search=${encodeURIComponent(search)}`, {
                headers: {
            ...
            tbody.innerHTML = records.map(r => `
                <tr class="clickable-row" onclick="viewRecord('${escapeHtml(r.registry_num)}')" style="cursor: pointer;" title="Click to view/send">
                    ...
                    <td>${escapeHtml(r.date_of_marriage)}</td>
                    <td>${escapeHtml(r.first_name)}</td>
                    <td>${escapeHtml(r.middle_name)}</td>
                    <td>${escapeHtml(r.last_name)}</td>
                    <td>${escapeHtml(r.birth_date)}</td>
                    <td>${r.h_age || '-'}</td>
                    <td>${escapeHtml(r.wfirst_name)}</td>
                    <td>${escapeHtml(r.wmiddle_name)}</td>
                    <td>${escapeHtml(r.wlast_name)}</td>
                    <td>${escapeHtml(r.wbirth_date)}</td>
                    <td>${r.w_age || '-'}</td>

                    <td style="text-align: center;">${r.sent_count > 0 ? r.sent_count : ''}</td>
                    <td>${r.date_sent ? formatDate(r.date_sent) : '-'}</td>
                    <td style="text-align: center;">${r.has_attachment ? '<i class="fas fa-file-pdf pdf-icon"></i>' : ''}</td>
                </tr>
            `).join('');
            
        } catch (error) {
            tbody.innerHTML = `<tr><td colspan="15" class="text-center text-danger">Error: ${escapeHtml(error.message)}</td></tr>`;
        }
    }
    ...
    async function loadStats() {
        try {
            const response = await fetch('registry_api.php?action=get_stats&type=marriage', {
                headers: {
                    'X-CSRF-Token': CSRF_TOKEN
                }
            });
    ...
    function viewRecord(registryNum) {
        window.location.href = `marriage_viewer.php?id=${encodeURIComponent(registryNum)}`;
    }
